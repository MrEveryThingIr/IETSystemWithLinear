...
return new class extends Migration
{
    ...
    {
        Schema::table('agreement_acceptances', function (Blueprint $table): void { $table->dropForeign(['group_agreement_version_id']); $table->foreign('group_agreement_version_id')->references('id')->on('group_agreement_versions')->cascadeOnDelete(); });
        Schema::table('admissions', function (Blueprint $table): void { $table->dropForeign(['source_invitation_id']); $table->foreign('source_invitation_id')->references('id')->on('group_invitations')->nullOnDelete(); });
    }
};
