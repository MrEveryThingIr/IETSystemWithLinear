...
class SystemManualContentTest extends TestCase
{
    ...
    public function test_manual_is_materialized_as_versioned_annotatable_content_and_is_idempotent(): void
    {
        ...
        $commitmentChapter = $first['chapters']->first(
            static fn ($content): bool => $content->activeRevisionRecord()?->title
                === SystemManualContent::CHAPTER_TITLES['commitments'],
        );
        $this->assertNotNull($commitmentChapter);

        $financialChapter = $first['chapters']->first(
            static fn ($content): bool => $content->activeRevisionRecord()?->title
                === SystemManualContent::CHAPTER_TITLES['financial-obligations'],
        );
        $this->assertNotNull($financialChapter);

        $journeysChapter = $first['chapters']->first(
            static fn ($content): bool => $content->activeRevisionRecord()?->title
        ...
        $communityChapter = $first['chapters']->first(
            static fn ($content): bool => $content->activeRevisionRecord()?->title
                === SystemManualContent::CHAPTER_TITLES['community'],
        );
        $this->assertNotNull($communityChapter);
        $this->assertSame('community', app(SystemManualHelpMap::class)->topicForRoute('groups.community'));
        $this->assertSame(
            SystemManualContent::CHAPTER_TITLES['community'],
            app(SystemManualHelpMap::class)->chapterTitle('community'),
        );

        $homeTodayChapter = $first['chapters']->first(
            static fn ($content): bool => $content->activeRevisionRecord()?->title
                === SystemManualContent::CHAPTER_TITLES['home-today'],
        );
        $this->assertNotNull($homeTodayChapter);
        $this->assertSame('home-today', app(SystemManualHelpMap::class)->topicForRoute('dashboard'));
        $this->assertSame(
            SystemManualContent::CHAPTER_TITLES['home-today'],
        ...
        $this->actingAs($reader->user)
            ->get(route('manual', ['topic' => 'commitments']))
            ->assertRedirect(
                route('contexts.contents.show', [
                    $first['context'],
                    $commitmentChapter,
        ...
        $this->actingAs($reader->user)
            ->get(route('manual', ['topic' => 'financial-obligations']))
            ->assertRedirect(
                route('contexts.contents.show', [
                    $first['context'],
                    $financialChapter,
        ...
        $this->actingAs($reader->user)
            ->get(route('manual', ['topic' => 'journeys']))
            ->assertRedirect(
                route('contexts.contents.show', [
                    $first['context'],
                    $journeysChapter,
        ...
        $this->actingAs($reader->user)
            ->get(route('manual', ['topic' => 'community']))
            ->assertRedirect(
                route('contexts.contents.show', [
                    $first['context'],
                    $communityChapter,
        ...
        $this->actingAs($reader->user)
            ->get(route('manual', ['topic' => 'home-today']))
            ->assertRedirect(
                route('contexts.contents.show', [
                    $first['context'],
                    $homeTodayChapter,
