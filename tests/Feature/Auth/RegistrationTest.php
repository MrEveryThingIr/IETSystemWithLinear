...
class RegistrationTest extends TestCase
{
    ...
    private function invitation(?string $email = null): GroupInvitation
    {
        $owner = Actor::factory()->create();
        $group = Group::create(['name' => 'Inviting group', 'created_by_actor_id' => $owner->id]);

        return GroupInvitation::create([
            'group_id' => $group->id,
            'invited_by_actor_id' => $owner->id,
            'email' => $email,
            'token' => 'registration-invitation-'.str()->random(8),
    ...
    public function test_registration_creates_active_unverified_account_and_logs_in(): void
    {
        ...
        Livewire::test(Register::class, ['token' => $invitation->token])->set($this->input())->call('register')
        ...
        $this->assertSame($user->id, $user->actor->user_id);
    ...
    public function test_registration_rejects_unconfirmed_password(): void
    {
        ...
        Livewire::test(Register::class, ['token' => $invitation->token])->set($this->input())->set('password_confirmation', 'different')
