<section class="space-y-6 sm:space-y-8">
    ...
    <div class="grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1.35fr)_minmax(16rem,0.65fr)]">
        <article class="min-w-0 overflow-hidden rounded-2xl border border-zinc-200 bg-white p-4 sm:p-6 dark:border-zinc-800 dark:bg-zinc-900">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <flux:button wire:click="openIdentityEditor" size="sm" variant="ghost" class="w-full shrink-0 sm:w-auto">
                    {{ __('ui.common.edit') }}
                </flux:button>
            </div>
        ...
        <article class="min-w-0 overflow-hidden rounded-2xl border border-zinc-200 bg-white p-4 sm:p-6 dark:border-zinc-800 dark:bg-zinc-900">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div class="flex min-w-0 items-center gap-4">
                    ...
                    <div class="min-w-0">
                        <h2 class="font-semibold">{{ __('ui.profile.display_image') }}</h2>
                        <p class="mt-1 text-sm text-zinc-500">
                            {{ trans_choice('ui.profile_sharing.item_count', $profile->images->count(), ['count' => $profile->images->count()]) }}
                        </p>
                    </div>
                </div>
                <flux:button wire:click="toggleMediaEditor" size="sm" variant="ghost" class="w-full shrink-0 sm:w-auto">
                    {{ __('ui.profile.images') }}
                </flux:button>
            </div>
        </article>
    </div>
    ...
    @if ($mediaEditorOpen)
        <section class="space-y-5 rounded-2xl border border-zinc-200 bg-white p-4 sm:p-6 dark:border-zinc-800 dark:bg-zinc-900">
                        ...
                        <article class="min-w-0 space-y-2" wire:key="profile-image-{{ $image->id }}">
                            ...
                            <div class="flex flex-col gap-1 sm:flex-row sm:flex-wrap">
                                ...
                                @elseif ($image->asset->isReadyForPublication())
                                    <flux:button wire:click="setDisplayImage({{ $image->id }})" size="xs" variant="ghost" class="w-full sm:w-auto">{{ __('ui.profile.use_as_display') }}</flux:button>
                                @endif
