    ...
    <x-app.page-header :title="__('presentation.title')" :description="$revision->title">
        <x-slot:actions>
            <div class="flex flex-wrap gap-2">
                <flux:button :href="route('contexts.contents.studio', [$context, $content])" variant="ghost">{{ __('presentation.back_to_studio') }}</flux:button>
                @if ($content->status === 'published')
                    <flux:button :href="route('contexts.contents.show', [$context, $content])" variant="ghost">{{ __('studio.back_to_reader') }}</flux:button>
                @endif
            </div>
        </x-slot:actions>
    </x-app.page-header>
