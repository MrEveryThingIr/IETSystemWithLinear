...
{
    public function execute(User $issuer, ?string $email=null, int $maxUses=1, int $expiresInDays=14): AccessInvitation
    {
        ...
        if ($email!==null) {
            abort_if(User::query()->whereRaw('LOWER(email) = ?',[$email])->exists(),422,__('access.already_registered'));
        }
        $actor=Actor::query()->where('user_id',$issuer->id)->firstOrFail();
        return DB::transaction(fn (): AccessInvitation => AccessInvitation::query()->create([
