...
class GroupRoleProvisioner
{
    ...
    public function provision(Group $group): array { $this->seedPermissions(); return $this->withinGroup($group, function (): array { $owner = $this->findOrCreateRole('Owner'); $member = $this->findOrCreateRole('Member'); $owner->syncPermissions(self::OWNER_PERMISSIONS); $member->syncPermissions(self::MEMBER_PERMISSIONS); return compact('owner', 'member'); }); }
    ...
    public function createRole(Group $group, string $name, array $permissions): Role { $this->seedPermissions(); return $this->withinGroup($group, function () use ($name, $permissions): Role { $role = Role::create(['name' => $name, 'guard_name' => 'web']); $role->syncPermissions($permissions); return $role; }); }
    ...
    private function findOrCreateRole(string $name): Role { return Role::findOrCreate($name, 'web'); }
