...
class GroupRoleProvisioner
{
    ...
    public function provision(Group $group): array
    {
        ...
        return $this->withinGroup($group, function () use ($group): array {
            $owner = $this->findOrCreateRole($group, 'Owner');
            $member = $this->findOrCreateRole($group, 'Member');
    ...
    public function createRole(Group $group, string $name, array $permissions): Role
    {
        ...
        return $this->withinGroup($group, function () use ($group, $name, $permissions): Role {
            $role = $this->findOrCreateRole($group, $name);
    ...
    public function deleteRole(Group $group, Role $role): void
    {
        $this->withinGroup($group, function () use ($role): void {
            $this->assertManagedRole($role);
            abort_if($role->users()->exists(), 422, 'Reassign members before deleting a role.');
            $role->delete();
        });
    }
    ...
    public function clear(Actor $actor, Group $group): void
    {
        $this->withinGroup($group, fn () => $actor->syncRoles([]));
    }
    ...
    private function findOrCreateRole(Group $group, string $name): Role
    {
        Role::findOrCreate($name, 'web');

        return Role::query()->where('group_id', $group->id)->where('name', $name)->where('guard_name', 'web')->firstOrFail();
    }
