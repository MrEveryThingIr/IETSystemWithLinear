        ...
        return DB::transaction(function () use ($occurrence, $current, $status, $eventType): PlanOccurrence {
            ...
            if ($status === PlanOccurrenceStatus::InProgress) {
                ...
                abort_unless(
                    $locked->canStart($now),
                    422,
                    $now->lt($locked->window_start_at)
                        ? 'This occurrence is not ready yet. Wait until its allowed early-start window opens.'
                        : 'The allowed start window has already closed.',
                );
                $actualStart = $now;
            } elseif ($status === PlanOccurrenceStatus::Completed) {
                abort_unless($locked->status === PlanOccurrenceStatus::InProgress, 422, 'Start the occurrence before completing it so actual start and end times remain truthful.');
                $actualEnd = $now;
                $completedAt = $now;
