...
class GroupInvitationController
{
    ...
    public function accept(Request $request, string $token, RedeemGroupInvitation $redemption): RedirectResponse
    {
        ...
        return to_route('groups.show', $admission->group_id)->with('status', 'Your admission application has been created or resumed. Membership is granted only after approval and agreement acceptance.');
    }

    private function findUsableInvitation(string $token): GroupInvitation
    {
        return GroupInvitation::query()->with('group')->where('token', $token)->whereNull('revoked_at')->where(function ($query): void { $query->whereNull('expires_at')->orWhere('expires_at', '>', now()); })->where(function ($query): void { $query->whereNull('max_uses')->orWhereColumn('uses_count', '<', 'max_uses'); })->firstOrFail();
    }
}
