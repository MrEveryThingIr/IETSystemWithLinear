...
class Admission extends Model
{
    ...
    public const STATUSES = ['draft', 'submitted', 'clarification_required', 'under_review', 'approved', 'rejected', 'cancelled'];
    ...
    {
        abort_unless(in_array($status, self::STATUSES, true), 422, 'Invalid admission status.');
        abort_if(in_array($this->status, ['rejected', 'cancelled'], true) && $status !== 'draft', 422, 'Terminal admissions must be resumed as drafts.');
