...
The first-publication experience hardening checkpoint now adds an idempotent verified-user personal accounting foundation, a configurable default monetary unit, safe local seeding with model/domain invariants enabled, a localized live date/time + ambient guidance strip, and a fractal Planner calendar that drills from year to month to day to hour, then into configurable 60/30/15/5/1-minute slots with exact date/time/duration-prefilled Plan creation. The accounting bootstrap creates standard accounts only and never fabricates an opening balance or JournalEntry. Changing the default currency never rewrites or converts prior financial history.

Profile-aware temporal presentation is now a cross-system invariant. Canonical database dates/timestamps remain stable, while every audited user-facing date/date-time surface resolves the signed-in User's locale, selected calendar and timezone. Persian/Jalali and Umm al-Qura calendars render as the primary civil calendar; when a non-Gregorian calendar is selected, a clearly labelled Gregorian equivalent is shown as a secondary reference. Date-only values never shift when timezone changes, while true instants are converted to the selected timezone. Planner month/year navigation uses the selected calendar's real boundaries rather than relabelled Gregorian boundaries. Calendar-aware input components replace native Gregorian date/datetime controls across the audited workflows. PHP `intl` is therefore a runtime requirement and is enforced in CI.

The hardening pass also fixes the Planner authorization-before-result-cap defect: authorization/context filtering now occurs before the visible result cap, so another Context's records cannot crowd legitimate records out of the result window. Focused regression coverage is in `tests/Feature/FirstPublicationExperienceTest.php`. PR #28 is the remote release-gate carrier for this checkpoint.
