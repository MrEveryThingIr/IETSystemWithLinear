...
The first-publication experience hardening checkpoint now adds an idempotent verified-user personal accounting foundation, a configurable default monetary unit, safe local seeding with model/domain invariants enabled, a localized live date/time + ambient guidance strip, and a fractal Planner calendar that drills from year to month to day with hourly inspection and date-prefilled Plan creation. The accounting bootstrap creates standard accounts only and never fabricates an opening balance or JournalEntry. Changing the default currency never rewrites or converts prior financial history.
